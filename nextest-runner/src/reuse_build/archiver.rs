...
use crate::{
    config::{get_num_cpus, FinalConfig, NextestProfile},
...
impl<'a, W: Write> Archiver<'a, W> {
    ...
    fn archive(mut self) -> Result<(W, usize), ArchiveCreateError> {
        ...
        for path in &self.archive_include.clone() {
            let src_path = self.binary_list.rust_build_meta.target_directory.join(path);
            let src_path = self.path_mapper.map_binary(src_path);

            let rel_path = Utf8Path::new("target").join(path);
            let rel_path = convert_rel_path_to_forward_slash(&rel_path);
